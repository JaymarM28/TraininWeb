...
export function SparkleParticles({
  ...
  const [activeColor, setActiveColor] = useState(particleColor || "#ffffff");
  ...
  useEffect(() => {
    ...
    const observer = new MutationObserver(() => setActiveColor(resolveThemeColor()));
  ...
  const mergedOptions: RecursivePartial<IOptions> = {
    ...
    fullScreen: { enable: false, zIndex: zIndexLevel },
    fpsLimit: 300,
    interactivity: {
      events: {
        ...
        onHover: {
          ...
          mode: hoverMode,
          parallax: { enable: enableParallax, force: 60, smooth: 10 },
        },
        resize: { enable: true },
      },
      modes: { push: { quantity: particleCount }, repulse: { distance: 200, duration: 0.4 } },
    },
    particles: {
      ...
      move: {
        ...
        speed: { min: minMoveSpeed || maxSpeed / 130, max: maxSpeed },
        straight: true,
      },
      collisions: { enable: enableCollisions, mode: "bounce" as const, bounce: { horizontal: { value: 1 }, vertical: { value: 1 } } },
      number: { value: baseDensity },
      opacity: { value: { min: minParticleOpacity || maxOpacity / 10, max: maxOpacity }, animation: { enable: true, sync: false, speed: opacityAnimationSpeed } },
      size: { value: { min: minParticleSize || maxParticleSize / 1.5, max: maxParticleSize } },
    },
    detectRetina: true,
    ...userOptions,
  };
  ...
  return isEngineReady ? (
    <Particles id={instanceId} options={mergedOptions} className={className} />
  ) : null;
}
